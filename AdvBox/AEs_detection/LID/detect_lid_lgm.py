...
def main(args):
    ...
    model.eval()
    #load lgm model    
    model_lgm = paddle.vision.models.resnet34(pretrained=False, num_classes=10)
    state_dict = paddle.load("./checkpoints/final_lgm/final.pdparams")
    model_lgm.set_state_dict(state_dict)
    model_lgm.eval()
    ...
    preds_normal = model(X_test)
    preds_noisy = model(X_test_noisy)
    preds_neg = np.concatenate((preds_normal, preds_noisy))
    ...
    index = np.where(pred_lgm_label != pred_adv_label)[0] # 为1则为对抗样本
    adv_num_sample = int(y_label_pred.shape[0] / 2)
    pred_adv[index] = pred_lgm[index]
    ...
    ## Evaluate detector on adversarial attack
    # 对x_test再次采用lgm优化的模型进行分类，对y_label_pred的值进行修改
    y_label_pred[adv_num_sample:][index] = 1
    index1 = np.where(y_pred[adv_num_sample:][index] <= 0.5)[0]
    y_pred[adv_num_sample:][index1] = 1. -  y_pred[adv_num_sample:][index1]
