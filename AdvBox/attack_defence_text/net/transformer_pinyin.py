...
class Encoder(nn.Module):
    def __init__(self, pinyin_hid_dim, embed_dim, num_heads, num_layers, ff_dim, dropout, max_len):
    ...
    def forward(self, x, pinyin_mask, mask):
        seq_len = x.size(1)
...
class TransformerClassifier(nn.Module):
    def __init__(self, pinyin_hid_dim, embed_dim, num_heads, num_layers, ff_dim, num_classes, dropout, max_len):
        ...
        self.encoder = Encoder(pinyin_hid_dim, embed_dim, num_heads, num_layers, ff_dim, dropout, max_len)
        self.fc = nn.Linear(embed_dim, num_classes)
        self.dropout = nn.Dropout(dropout)

    def forward(self, input_ids,pinyin_mask, attention_mask):
        encoder_output = self.encoder(input_ids, pinyin_mask, attention_mask)
