...
class HopSkipJumpAttack(Attack):
    ...
    def _apply(self, adversary, steps=100):
        ...
        for i in range(steps):
            ...
            perturbed = self.hsja(
                perturbed,
                adversary,
                target_label=self.addtional_params['target_label'],
                target_image=self.addtional_params['target_image'],
                constraint=self.addtional_params['constraint'],
                num_iterations=self.addtional_params['num_iterations'],
            )
            ...
            print("adv_label:", adv_label, "score:", self.model.predict(perturbed_normalized)[0][adv_label])
    ...
    def decision_function(self, images, params):
        ...
        for j in range(c):
            images[0, j] = old_div(images[0, j] - mean[j], std[j])
    ...
    def approximate_gradient(self, sample, num_evals, delta, params):
        ...
        h = time.time()
        # print("b - a:{}, c - b:{}, d - c: {}, e - d:{}, f - e: {}, g - f: {}, h - g: {}".format(b - a, c - b, d - c, e - d, f - e, g - f, h - g))
        return gradf
