...
class LOGITS_DISPERSION(Attack):
    ...
    Including:
    * softmax_kl_l_inf, softmax of logits of L2 distance KL dispersion
    * softmax_kl_l_2, softmax of logits of L inf distance KL dispersion
    * logits_norm_l_2, logits of norm 2 distance dispersion ||logit - logits_adv||22
    * difference_logits_ratio, Difference of Logits Ratio Loss. DLR(x, y) = − (zy − max(i!=y)zi) / (zπ1 − zπ3)
    ...
    """
    def __init__(self, model, dispersion_type=None):
        super(LOGITS_DISPERSION, self).__init__(model)
        self.model = model
        ...
        assert self.box_constrains_upper_bound > self.box_constrains_lower_bound

        support_type = ('softmax_kl_l_inf', 'softmax_kl_l_2', 'logits_norm_l_2', 'difference_logits_ratio')
        assert dispersion_type in support_type
    ...
    def _apply(self,
        ...
        # TODO: epsilon to be transform from user input normalization settings.
        """
        Launch an attack process.
        Args:
            adversary: Adversary. An adversary instance with initial status.
            epsilon: float32. The perturbation lower upper range.
        ...
        if dispersion_type == 'softmax_kl_l_inf':
            ...
            for _ in range(perturb_steps):
                ...
                # TODO: check these two methods. Are they the same?
        ...
        # TODO: finsh this.
        elif dispersion_type == 'softmax_kl_l_2':
