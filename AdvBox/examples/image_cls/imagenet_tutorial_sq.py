...
"""
Attacking tutorial on imagenet using the Square attack tool
...
def main(orig):
    ...
    # non-targeted attack
    attack_config = {"target": args.target, "max_steps": args.max_steps, "eps": args.eps, "window_size": args.window_size}
    if args.norm == 'LInf':
        attack = SquareAttackInfinity(paddle_model, **attack_config)
    elif args.norm == 'L2':
        attack = SquareAttack2(paddle_model, **attack_config)
    else:
        raise ValueError("Invaild norm. Select one from 'L2' or 'LInf'")
    ...
    if adversary.is_successful():
        print(bcolors.RED + "Square Attack succeeded, adversarial_label: {}".format(\
