...
def gradient_descent_optim_attack(model, im):
    ...
    opt = paddle.optimizer.Adam(learning_rate= 1e-2, parameters = [delta])
    adv_img = im + delta
...
def predict_adv_pgd(model,
    ...
    return save_dir+'/adv_'+im_file, pred_saved_path


def predict_adv_cw(model,
            model_path,
            transforms,
            image_list,
            image_dir=None,
    ...
    for i, im_path in enumerate(img_lists[local_rank]):
        data = preprocess(im_path, transforms)
        adv_img = gradient_descent_optim_attack(model, data['img'])  
