    ...
    def is_adversarial(self, predictions, annotation):
        ...
        if self._model_name.startswith("pytorchhub_"):
            ...
            if predictions.pred[0][0, 5] != annotation:
                return True
            return False
        elif self._model_name.startswith("paddlehub_") or self._model_name.startswith("paddledet_"):
            _threshold = 0.1
            tgt_class_idx = np.argwhere(predictions[:, 0] == self._target_class)
            tgt_class_prob = predictions[tgt_class_idx, 1]
            return np.all(tgt_class_prob < _threshold)
