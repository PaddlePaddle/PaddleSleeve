...
Contains:
* Initialize a ppyolo detector and inference pictures.
...
def run(FLAGS, cfg):   
    ...
    cv2.imwrite(FLAGS.output_dir+'/ppyolo_adverse_'+ FLAGS.infer_img.split('/')[-1], data_adv)
    cv2.imwrite(FLAGS.output_dir+'/ppyolo_advpatch_'+ FLAGS.infer_img.split('/')[-1], adv_patch)  
    # result visualization
    _draw_result_and_save(FLAGS.output_dir+'/ppyolo_adverse_'+ FLAGS.infer_img.split('/')[-1],FLAGS.output_dir+'/out_', outs_adv, data0, datainfo0, draw_threshold)
