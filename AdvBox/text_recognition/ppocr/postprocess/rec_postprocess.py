...
class BaseRecLabelDecode(object):
    ...
    def __init__(self, character_dict_path=None, use_space_char=False):
        ...
        if character_dict_path is None:
            self.character_str = "0123456789abcdefghijklmnopqrstuvwxyz" # total 36
    ...
    def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
        ...
        batch_size = len(text_index) # batch_size: 1, text_index: (1, 80)
        for batch_idx in range(batch_size):
            char_list = []
            conf_list = []
            for idx in range(len(text_index[batch_idx])):# 80
                if text_index[batch_idx][idx] in ignored_tokens:
                    continue
                
                if is_remove_duplicate: #True
                    # only for predict
                    if idx > 0 and text_index[batch_idx][idx - 1] == text_index[
                            batch_idx][idx]:
                        continue
                #print(self.character)
                #print("idx=====", idx, int(text_index[batch_idx][
                #    idx]), self.character[int(text_index[batch_idx][idx])])
...
class CTCLabelDecode(BaseRecLabelDecode):
    ...
    def __call__(self, preds, label=None, *args, **kwargs):
        ...
        if isinstance(preds, paddle.Tensor):
            preds = preds.numpy()
        #print("preds=====", preds.shape) # 1, 80, 6625
        preds_idx = preds.argmax(axis=2) # 1, 80
        preds_prob = preds.max(axis=2) # 1, 80
        #print("idx, prob=====", preds_idx)
        #print("index=======", np.nonzero(preds_idx))
