...
def getLane_simlane(prob_map, y_px_gap, pts, thresh, resize_shape=None):
    """
    Arguments:
    ----------
...
def prob2lines_simlane(seg_pred, exist, resize_shape=None, smooth=True, y_px_gap=10, pts=None, thresh=0.3):
    """
    Arguments:
    ----------
    ...
    for i in range(4):
        ...
        if exist[i] > 0:
            coords = getLane_simlane(prob_map, y_px_gap, pts, thresh, resize_shape)
            if (coords>0).sum() < 2:
                continue
            coordinates.append(
