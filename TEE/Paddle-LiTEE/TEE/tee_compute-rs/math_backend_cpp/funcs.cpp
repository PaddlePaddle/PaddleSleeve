...
template <typename T>
void move_data(T *mtx, int i, int m, int n) {
    T tmp = mtx[i];
    int cur = i;
...
extern "C" void transpose_int(int8_t* dout, int m, int n) {
    for (int i = 0; i < m * n; ++i) {
        int next = get_next(i, m, n);
        while (next > i)
            next = get_next(next, m, n);
        if (next == i)
            move_data(dout, i, m, n);
    }
}

extern "C" void transpose_float(float* dout, int m, int n) {
    for (int i = 0; i < m * n; ++i) {
        int next = get_next(i, m, n);
        while (next > i)
            next = get_next(next, m, n);
